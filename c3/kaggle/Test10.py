...
class Test10:
    ...
    def k_fold(self, k, X_train, y_train, num_epochs,
           learning_rate, weight_decay, batch_size):
        train_l_sum, valid_l_sum = 0, 0
        for i in range(k):
            data = self.get_k_fold_data(k, i, X_train, y_train)
            net = self.get_net()
            train_ls, valid_ls = self.train(net, *data, num_epochs, learning_rate,
    ...
    def generate(self):
        ...
        print('%d-fold validation: avg train rmse %f, avg valid rmse %f'
              % (k, train_l, valid_l))
        self.train_and_pred(train_features, test_features, train_labels, test_data,
               num_epochs, lr, weight_decay, batch_size)
